...
#### 0. Billing Service(gRPC) - хранит историю платежей, валидирует токены, связывает SSO и PAYMENT, взаимодействует с PAYMENT через mTLS(+возможно API-key):
- фсё)
...
Сначала хотел просто побаловаться с REST API, потом чуть идея обраслась, ну и надо было с gRPC поработать. В итоге вот на gRPC только SSO(upd и billing), а остальное пока по RESTу ну и планируется также, но может быть что-то или вообще всё переведу на gRPC, но пока таких планов нет.
## Сабмодули какие-то...
