...
- [Схема транзакций](#схема-транзакций)
  - [On-Us Transfer, Direct Debit](#on-us-transfer-direct-debit)
  - [On-Us FX Transfer](#on-us-fx-transfer)
  - [Outbound Remittance, Outbound Direct Debit](#outbound-remittance-outbound-direct-debit)
  - [Inbound Remittance, Inbound Direct Debit](#inbound-remittance-inbound-direct-debit)
...
Отвечает за конвертацию валют при межвалютных переводах. Работает по двухфазной модели:
- **Котировка (quote)** - резервирует курс на ограниченное время без реального движения средств. Вызывается на этапе превью платежа, чтобы показать пользователю финальную сумму до подтверждения.
- **Активация (activate)** - фиксирует курс на время выполнения саги. Деньги всё ещё не двигаются.
- **Коммит (commit)** - реальное движение средств по корреспондентским счетам после успешного завершения всех шагов.
- **Отмена (cancel)** - освобождение курса без движения средств, если сага прервалась.
Пока не реализован.

### Fee Service

Рассчитывает комиссию за операции: процент от суммы, фиксированная плата или их комбинация. Вызывается на этапе превью платежа, результат передаётся в TransactionPayload и доезжает до Account Service, который распределяет комиссию на счёт доходов при списании.
Пока не реализован.

### Mandate Service

Хранит мандаты — разрешения на прямое списание средств. При создании Direct Debit-транзакции API Gateway или Switch Service проверяет действительность мандата через этот сервис до запуска саги. Сам Transaction Service ничего о мандатах не знает — для него это обычный платёж с дополнительным идентификатором в parties.
Пока не реализован. 
...
Перевод между двумя внутренними счетами с конвертацией валюты.

1. Заморозка средств отправителя в исходной валюте
2. Активация FX-сделки (фиксация курса)
3. Пополнение счёта получателя в целевой валюте
4. Списание замороженных средств отправителя
5. Коммит FX-сделки (реальное движение по коррсчетам)
6. Завершение

При отказе на шаге 1 — отмена транзакции. На шаге 2 — разморозка средств отправителя (котировка не активирована, протухнет по таймауту). На шаге 3 — отмена FX-сделки и разморозка отправителя. На шаге 4 — возврат средств получателю, отмена FX-сделки и разморозка отправителя. На шаге 5 — блокировка обоих счетов (FX-сделка закоммичена, требуется ручное расследование). Если компенсация падает — блокировка счетов.
...
#### Refund On-Us FX — возврат по FX-переводу

Обратная конвертация по новому курсу, полученному на этапе превью.

1. Списание с получателя в целевой валюте
2. Активация обратной FX-сделки
3. Пополнение счёта отправителя в исходной валюте
4. Коммит FX-сделки
5. Завершение

При отказе на любом шаге — блокировка соответствующего счёта. Комиссия за исходный перевод не возвращается.
...
#### Chargeback On-Us FX — чарджбек по FX-переводу

Аналогичен Refund On-Us FX, но инициируется внешней стороной.
