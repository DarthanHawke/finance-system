...
type EventHandler interface {
	HandleExternalRequest(ctx context.Context, event *models.Event) error
	HandleExternalCommit(ctx context.Context, event *models.Event) error
	HandleExternalRollback(ctx context.Context, event *models.Event) error
}
...
func (c *Consumer) routeEvent(ctx context.Context, event *models.Event) error {
	switch event.Type {
	case models.EventExternalRequest:
		return c.handler.HandleExternalRequest(ctx, event)
	case models.EventExternalCommit:
		return c.handler.HandleExternalCommit(ctx, event)
	case models.EventExternalRollback:
		return c.handler.HandleExternalRollback(ctx, event)
	default:
